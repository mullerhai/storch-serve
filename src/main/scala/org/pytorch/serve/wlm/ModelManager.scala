...
final class ModelManager private( val configManager: ConfigManager,  val wlm: WorkLoadManager) {
  ...
  final private var modelsNameMap: ConcurrentHashMap[String, ModelVersionedRefs] = new ConcurrentHashMap[String, ModelVersionedRefs]
  final private var startupModels: util.HashSet[String] = new util.HashSet[String]
  ...
  private def setupModelVenv(model: Model): Unit = {
    ...
    commandParts.add(Paths.get(configManager.getModelServerHome, "ts", "utils", "setup_model_venv.py").toAbsolutePath.toString)
  ...
  private def createVersionedModel(model: Model, versionId: String): Unit = {
    ...
    if (modelVersionRef == null) modelVersionRef = new ModelVersionedRefs
  ...
  def unregisterModel(modelName: String, versionIdTmp: String, isCleanUp: Boolean): Int = {
    ...
    var versionId = if (versionIdTmp == null) then vmodel.getDefaultVersion else versionIdTmp
    ...
    try {
      ...
      if (httpResponseStatus == HttpURLConnection.HTTP_OK) {
        ...
        if (versionId == null) versionId = vmodel.getDefaultVersion
        vmodel.addVersionModel(model, versionId)
      }
      if (vmodel.getAllVersions.size == 0) modelsNameMap.remove(modelName)
  ...
  private def getVersionModel(modelName: String, versionId: String) = {
    ...
    vmodel.getVersionModel(versionId)
  }
  ...
  def getDefaultModels(skipFuntions: Boolean): util.Map[String, Model] = {
    val defModelsMap = new ConcurrentHashMap[String, Model]
//    import scala.collection.JavaConversions._
    for (key <- modelsNameMap.keySet.asScala) {
      val mvr = modelsNameMap.get(key)
      if (mvr != null) {
        val defaultModel = mvr.getDefaultModel
  ...
  def getDefaultModels: util.Map[String, Model] = getDefaultModels(false)

  def getWorkers(modelVersionName: ModelVersionName): util.List[WorkerThread] = wlm.getWorkers(modelVersionName)

  def getWorkers: util.Map[Integer, WorkerThread] = wlm.getWorkers
  ...
  def scaleRequestStatus(modelName: String, versionId: String): Boolean = {
    val model = modelsNameMap.get(modelName).getVersionModel(versionId)
    var numWorkers = 0
  ...
  def getStartupModels: util.Set[String] = startupModels

  @throws[ModelVersionNotFoundException]
  def getModel(modelName: String, versionId: String): Model = {
    val vmodel = modelsNameMap.get(modelName)
    if (vmodel == null) return null
    val model = vmodel.getVersionModel(versionId)
    if (model == null) throw new ModelVersionNotFoundException("Model version: " + versionId + " does not exist for model: " + modelName)
    else model
  }

  @throws[ModelNotFoundException]
  def getAllModelVersions(modelName: String): util.Set[util.Map.Entry[String, Model]] = {
    val vmodel = modelsNameMap.get(modelName)
    if (vmodel == null) throw new ModelNotFoundException("Model not found: " + modelName)
    vmodel.getAllVersions
  }

  def getAllModels: util.Set[util.Map.Entry[String, ModelVersionedRefs]] = modelsNameMap.entrySet
